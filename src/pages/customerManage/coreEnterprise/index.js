...
class Info extends Component {
	...
	getListData = async (params = {}) => {       
        ...
        params['enterpriseCategory'] = 1;
        
        delete params.distrDt;
        let response = await request({url: 'register/list', method: 'get', params}).then(res => {
        	if(res.code === 0) {
    ...
    // 粮票查看详情
	showDetail(record) {
		this.props.history.push(`/customer/core/${record.enterpriseId}`)
	}
	// 分页切换
	...
	render() {
		const {userStatusList, companyInfo} = this.props;
		       ...
		       <Col span={24}>
		         <Card title='核心企业管理' size="small">
				   <SearchForm wrappedComponentRef={form=>{this.searchForm = form}} sta= {this.state} setState={this.setState.bind(this)} getListData={this.getListData}/>   
				   	{
					   	companyInfo.category === 3 ? null 
					   		: <Button style = {{marginTop: '10px', marginBottom: '10px'}} onClick = {() => this.props.history.push('/customer/addcore')}>
						   		新增核心企业
						   	</Button>
					}
				   <RcTable 
...
export default class CoreEnterprise extends Component {	
	render() {
		return (<div>
			<CustomInfo/>
		</div>)
